...
{
    char *ptr = rte_pktmbuf_mtod (buf, char *);
    if (ether_hdr) *(rte_ether_hdr *)ptr = *ether_hdr;
    if (ipv4_hdr) *(rte_ipv4_hdr *)(ptr + sizeof (rte_ether_hdr)) = *ipv4_hdr;
    if (udp_hdr) *(rte_udp_hdr *)(ptr + sizeof (rte_ether_hdr) + sizeof (rte_ipv4_hdr)) = *udp_hdr;
